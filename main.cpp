...
std::map<coord, bool> solve_equations(std::vector<std::set<coord>> equations_) {
	...
	for (auto var : variables) {
		printf("Solving equations %d/%d\r", progress++, (int)variables.size()); fflush(stdout);
		tmp[var] = -1;
		for (int j = 0; j < m; j++) {
	...
	//printf("Bad equations:%d\n", bad_equations); fflush(stdout);
	//err = bad_equations;
	return ret;
}

bool try_ver_lv(int ver, int lv, const std::string& str1, const std::string& str2,const char* save_path) {
	int n = ver * 4 + 17;
	int mask;
	...
	int err1 = seg1.size() / 8;
	int err2 = seg2.size() / 8;
	...
	if (err1 >= block_info[ver - 1][lv].tolerance || err2 >= block_info[ver - 1][lv].tolerance) {
		return false;
	}
	iter = 0;
	...
	for (int g = 0; g < code_groups.size(); g++) {
		auto cg = code_groups[g];
		auto table = RS::generate_table(RS::generate_poly(cg.second.size()), cg.first.size());
					...
					for (int k = 0; k < 8; k++) {
						if (table[cg.second.size() * 8 - 1 - (c_id * 8 + j)][cg.first.size() * 8 - 1 - (d_id * 8 + k)]) {
							...
							if (mat1[c.second][c.first] != -1) {
								b1 = b1 ^ mat1[c.second][c.first];
							}
							else if (mat2[c.second][c.first] != -1) {
								b1 = b1 ^ mat2[c.second][c.first];
							}
							else {
								equation1.insert(c);
							}
							...
							if (mat2[mc.second][mc.first] != -1) {
								b2 = b2 ^ mat2[mc.second][mc.first];
							}
							else if (mat1[mc.second][mc.first] != -1) {
								b2 = b2 ^ mat1[mc.second][mc.first];
							}
							else {
								equation2.insert(mc);
							}
	...
	for (int g = 0; g < code_groups.size(); g++) {
		...
		if (!failing) {
			...
			if (!failing) {
				...
				if (e1 > block_info[ver - 1][lv].tolerance || e2 > block_info[ver - 1][lv].tolerance)failing = true;
			}
		}
		...
		//Decoder::decode(rs_encoded_block.data(), decoded_block.data(), cg.first.size() + cg.second.size(), cg.first.size());
	}
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			if (prefilled[i][j] == -1) {
				if (merged[i][j] != -1)
					merged[i][j] ^= masking_functions[mask](j, i);
			}
		}
	}
...
int main()
{
	...
	for (int ver = 4; ver <= 40; ver++) {
		for (int lv = 0; lv < 4; lv++) {
		//for (int lv = 0; lv < 1; lv++) { // only try Level L
			printf("Trying Version %d, Level %c\n", ver, lvs[lv]);
			if (try_ver_lv(ver, lv, "HTTPS://WWW.GOOGLE.COM", "HTTPS://BING.COM", "F:\\search.jpg")) {
				return 0;
			}
		}
	}
